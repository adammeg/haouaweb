...
export async function POST(req: Request) {
  ...
  const rl = rateLimitMemory(`clinic_login:${ip}`, 20, 60_000);
  if (!rl.ok) {
  ...
  try {
    ...
    if (doctor.role !== "clinic_admin" || !doctor.clinicId) {
      return NextResponse.json({ error: "not_clinic_admin" }, { status: 403 });
    }
